...
def verify_single_trajectory_worker(args):
    ...
    # 构造障碍物地图文件路径
    obstacle_map_path = (
        project_root / "assets" / "eval_scenarios_20" / 
...
def load_trajectories(pkl_path=None):
    """加载保存的轨迹"""
    if pkl_path is None:
        pkl_path = Path(__file__).parent.parent / "assets" / "trajectories_lightweight_20.pkl"
...
def main():
    ...
    # 保存结果
    output_path = Path(__file__).parent.parent / "assets" / "reachability_results_pure_polar_lightweight_20.json"
