...
"""
POLAR 可达性验证主函数
支持动态网络结构（原始TD3: 25→800→600→2, 轻量级: 25→26→26→2）
"""
...
def extract_actor_weights(actor):
    ...
    Args:
        actor: TD3 的 Actor 网络（支持不同结构）
    
    Returns:
        weights: list of numpy arrays，例如 [(hidden1, input), (hidden2, hidden1), (output, hidden2)]
        biases: list of numpy arrays，例如 [(hidden1,), (hidden2,), (output,)]
    """
    weights = []
    biases = []
...
def compute_reachable_set(
    ...
    # 打印网络结构信息（首次调用时）
    if not hasattr(compute_reachable_set, '_structure_printed'):
        network_structure = f"{state_dim}"
        for w in weights:
            network_structure += f" → {w.shape[0]}"
        print(f"[POLAR] 检测到网络结构: {network_structure}")
        compute_reachable_set._structure_printed = True
    ...
    num_layers = len(biases)  # 动态适配网络层数
